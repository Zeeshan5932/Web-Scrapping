#!/usr/bin/env python3
# filepath: f:\eFaida\news_aggregator\business.py
"""
BBC Business News Scraper
This script scrapes business news articles from the BBC Business section and saves them to a JSON file.
"""
            ...
            try:
                ...
                if headline:
                    articles.append({
                        ...
                        "image_url": image_url,
                        "date": metadata["date"],
                        "category": metadata["category"] or "Business",
...
def fetch_business_news(output_file="data/business_details.json", fetch_full_articles=True, delay=1):
    """Fetch BBC business news, extract articles, and save to JSON file"""
    ...
    try:
        ...
        print(f"Fetching business news from {url}...")
        response = requests.get(url, headers=headers, timeout=10)
        
        if response.status_code != 200:
            print(f"Failed to fetch business news: Status code {response.status_code}")
            return False
        ...
        print(f"Successfully saved {len(articles)} business articles to {output_file}")
        return True
        
    except Exception as e:
        print(f"Error fetching BBC business news: {e}")
        return False
...
if __name__ == "__main__":
    ...
    # Fetch business news and save to JSON file in root directory
    output_file = "data/business_details.json"
    success = fetch_business_news(
        output_file=output_file,
        fetch_full_articles=True,  # Set to False for quicker execution if you only want headlines
        delay=2  # Delay between article requests to be respectful to the server
    )
    
    if success:
        print(f"Business news data saved to {output_file}")
    else:
        print("Failed to fetch business news data.")
