#!/usr/bin/env python3
# filepath: f:\eFaida\news_aggregator\sports.py
"""
BBC Sports News Scraper
This script scrapes sports news articles from the BBC Sport section and saves them to a JSON file.
"""
...
def extract_metadata(card_element):
    ...
    metadata = {"date": "", "category": "", "sport_type": ""}
    ...
    try:
        ...
        # Try to extract sport type (football, cricket, etc.)
        sport_type_span = card_element.select_one('.ssrcss-1c8xj2y-MetadataLink')
        if sport_type_span:
            metadata["sport_type"] = sport_type_span.get_text(strip=True)
    except Exception as e:
...
def extract_articles(soup):
    ...
    card_types = [
        ...
        '.gs-c-promo'  # Additional selector specific to sports page
    ]
    
    # Debug - print number of cards found by selector
    ...
    for selector in card_types:
        ...
        for card in card_elements:
            try:
                # Extract headline
                headline_element = card.select_one('[data-testid="card-headline"]') or card.select_one('.gs-c-promo-heading__title')
                headline = headline_element.get_text(strip=True) if headline_element else ""
                
                # Extract summary/description
                description = card.select_one('[data-testid="card-description"]') or card.select_one('.gs-c-promo-summary')
                ...
                # Skip newsletter signup and non-article links
                if "signup" in link.lower() or not link or "bbc.com" not in link:
                    continue
                ...
                if headline:
                    articles.append({
                        ...
                        "image_url": image_url,
                        "date": metadata["date"],
                        "category": metadata["category"] or "Sport",
...
def fetch_sports_news(output_file="data/sports_details.json", fetch_full_articles=True, delay=1):
    ...
    try:
        ...
        # Fetch the sports page
        print(f"Fetching sports news from {url}...")
        response = requests.get(url, headers=headers, timeout=10)
        ...
        print(f"Found {len(articles)} sports articles.")
        ...
        print(f"Error fetching BBC sports news: {e}")
        return False
...
if __name__ == "__main__":
    ...
    # Fetch sports news and save to JSON file
    ...
    if success:
        print(f"Sports news data saved to {output_file}")
    else:
        print("Failed to fetch sports news data.")
